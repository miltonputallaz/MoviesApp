...
struct GenericRemoteRepository {
    ...
    func getSingleEntity<T: Codable>(url: String, completion: @escaping (T?, BackendError?) -> ()){
                    ...
                    case .success(let data):
                        if let code = response.response?.statusCode{
                            switch code {
                                case 200...299:
                                    do {
                                        let listResponse = try JSONDecoder().decode(T.self, from: response.data!)
                                        completion(listResponse, nil)
                                    } catch let parsingError {
                                        print("Error parsing")
                                    }
    ...
    func getImageFrom(url imageUrl: String, completion: @escaping (UIImage?, BackendError?) -> ()){
